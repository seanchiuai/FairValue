...
async function captureState({ label, page, expectedNames, appRegionMarker }) {
  await page.bringToFront();
  await delay(400);
  const aria = await page.locator('body').ariaSnapshot({ mode: 'ai' });
  const axRaw = captureMacAccessibilityTree();
  const appAxRaw = scopedAxTree(axRaw, appRegionMarker);
  const missing = expectedNames.filter((name) => !appAxRaw.includes(name));
  assert.deepEqual(
    missing,
    [],
    `${label} missing macOS AX names: ${missing.join(', ')}\n\nAX excerpt:\n${interestingAxLines(appAxRaw).join('\n')}`
  );
  return {
    label,
    expectedNames,
...
function renderReport({ frontendPort, backendPort, roomCode, captures }) {
  const tableRows = captures.map((capture) => (
    `| ${capture.label} | ${capture.expectedNames.map((name) => `\`${name}\``).join('<br>')} | PASS |`
  )).join('\n');
...
async function main() {
  ...
  try {
    ...
    captures.push(await captureState({
      ...
      expectedNames: ['Settle Market', 'Actual price', 'Cancel'],
    }));
