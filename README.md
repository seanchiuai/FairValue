...
- **Market template registry** — `/api/market-templates` publishes `market-template-registry/v1`, including the playable binary LMSR format and draft-only contracts for range, rent-yield, time-on-market, and renovation-budget markets. Range markets now have a generic multi-outcome LMSR pricing core, but draft-only formats remain blocked from room creation until settlement/replay/UI workflows exist.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. Only `binary_over_under` is marked `playable`; future formats are explicit `draft_only` contracts so clients can plan without pretending unsupported room workflows exist.
...
- `src/lib/multiOutcomeMarketEngine.js` provides the generic `lmsr_multi_outcome_v1` core for future range/ranked templates: n-outcome log-sum-exp cost, softmax probabilities, budget-to-shares execution, public state projection, and winning-outcome settlement math. It is intentionally not wired into live room creation yet.
...
`GET /api/market-templates` returns the versioned public registry of room market formats. `binary_over_under` is the only playable format today and uses `lmsr_binary_v1`; `range_price_band` is registered against the generic `lmsr_multi_outcome_v1` pricing core but remains draft-only until settlement inputs, replay rules, and UI workflows are implemented. `rent_yield_over_under`, `time_on_market_over_under`, and `renovation_budget_over_under` are also registered as draft-only contracts.
