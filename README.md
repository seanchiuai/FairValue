...
### Property PostGIS Projection

```bash
npm run data:postgres:properties
npm run data:postgres:properties -- --write
```

`data:postgres:properties` defaults to a no-side-effect dry run that normalizes the current property snapshot into `fairvalue.propertyPostgresProjection.v1`, reports the target PostGIS tables, sample property IDs, manifest hash, and static-provider limitations, and works without `DATABASE_URL`. Passing `--write` requires `DATABASE_URL`, creates `postgis` plus the `fairvalue_property_manifests` and `fairvalue_properties` tables if missing, and replaces only that projection's rows with public-safe normalized fields plus `geography(Point, 4326)` centroids. App routes still read the manifest-backed static snapshot by default; set `FAIRVALUE_PROPERTY_SNAPSHOT_STORE=postgres` only after writing the projection to make startup load `fairvalue_properties` into the same public-safe property query, neighborhood, and geospatial surfaces. If that explicit source is unavailable or empty, startup fails closed unless `FAIRVALUE_PROPERTY_POSTGRES_FALLBACK=static` is set. The older `FAIRVALUE_PROPERTY_QUERY_SOURCE=postgres` alias is still accepted.
...
- **Operator incident workflow** overlays persisted ops triage state on generated room incidents. `FAIRVALUE_OPERATOR_INCIDENT_WORKFLOW_PATH` or `.fairvalue/operator-incidents.json` stores redacted assignment, status, and timeline entries so incident review can survive local server restarts without storing host tokens, user tokens, private profile state, or raw evidence documents. `/ops/incidents` renders this queue, supports severity/room filters, runs ops-token-only redacted replay checks with hashed canonical event rows, and PATCHes workflow status, assignee, and redacted notes through the ops API.
- **Property data manifest** is generated by `scripts/property-data-manifest.js` into `public/data/property-data-manifest.json`. It treats the current Zillow/MLS export as a static provider snapshot, stores deterministic SHA-256 provenance, and gives frontend/server surfaces a typed freshness/coverage contract without claiming a live feed. `GET /api/properties` and `GET /api/properties/:propertyId` return a redacted property projection plus manifest provenance; `GET /api/geospatial/properties` and `GET /api/properties/:propertyId/nearby` expose public-safe provider centroids for radius, bounding-box, and nearby-property queries; `GET /api/neighborhoods` derives `fairvalue.neighborhoodIndex.v1` ZIP-code entities with directional aggregate metrics over the same snapshot, `GET /api/neighborhoods/:zipCode/market-drafts` generates a playable price-momentum scenario plus draft-only rent-yield/outperformance neighborhood scenarios from those baselines, and the neighborhood evidence adapter can request cited public-safe provider aggregate snapshots before settlement or future promotion work. The private alert evaluator reads the same snapshot, so threshold alerts are static-snapshot evaluations rather than provider push events. `FAIRVALUE_PROPERTY_SNAPSHOT_STORE=postgres` moves those read APIs to the PostGIS projection after an operator has written it, while preserving the same public-safe response contracts.
- **Property PostGIS projection** is defined in `server/propertyPostgresProjection.js` and operated by `scripts/property-postgres-projection.js`. It converts the static snapshot plus manifest into public-safe `fairvalue_property_manifests` and `fairvalue_properties` rows with a PostGIS `geography(Point, 4326)` centroid index, preserving dataset/source hashes without raw private provider payloads. `FAIRVALUE_PROPERTY_SNAPSHOT_STORE=postgres` is an explicit read-side adapter switch that loads those rows at startup; it is still a static snapshot projection, not a live provider feed.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. `binary_over_under`, `range_price_band`, `rent_yield_over_under`, `time_on_market_over_under`, `renovation_budget_over_under`, and `neighborhood_price_momentum_over_under` are playable in the rendered room UI, canonical event replay, settlement, and public verification. `neighborhood_price_momentum_over_under` uses binary LMSR with a ZIP median-price threshold and settles from `future_median_price`. `neighborhood_rent_yield_over_under` and `neighborhood_outperformance_over_under` remain explicit `draft_only` contracts: they can be generated as scenario drafts from ZIP baselines, but room creation rejects them until neighborhood-specific pricing, evidence ingestion, replay, and settlement workflows exist.
...
- **Structured Intelligence Provider Contract** is exposed at `GET /api/ai/intelligence/properties/:propertyId/contract`. It returns a redacted property context, the required `fairvalue.marketIntelligence.v2` output shape, required analyst roles, citation requirements, prohibited claims, and a deterministic request hash. `POST /api/ai/intelligence/properties/:propertyId/generate` can send that contract to `FAIRVALUE_INTELLIGENCE_PROVIDER_URL` with `FAIRVALUE_INTELLIGENCE_PROVIDER_API_KEY`, validate the response, and fall back to deterministic local intelligence when credentials are absent or output fails.
- **Neighborhood Evidence Provider Contract** is exposed per draft at `GET /api/neighborhoods/:zipCode/market-drafts/:draftId/evidence-contract`. `POST /api/neighborhoods/:zipCode/market-drafts/:draftId/evidence/generate` can send a redacted ZIP aggregate contract to `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_URL` with `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_API_KEY`, validate cited `fairvalue.neighborhoodEvidence.v1` aggregate evidence, and fall back to local gap assessment when credentials are absent or output fails. Property detail scenario cards render the adapter readiness. Accepted evidence can support price-momentum settlement review, while the remaining draft-only neighborhood formats still need pricing, replay, room creation, settlement UI, and dispute work before promotion.
- **Operator Review** is a host-facing deterministic recap surface over room state plus host-authorized event logs, including draft audit, settlement evidence, timeline, and integrity checks. `GET /api/ops/incidents` derives a redacted `fairvalue.operatorIncidentQueue.v1` triage queue for durability failures, missing settlement packets, locked unsettled rooms, missing event logs, and dispute-ready rooms; `/ops/incidents` renders that queue as an ops-token-aware workflow console with redacted replay projection checks for selected incidents.
...
- **`fairvalue_property_manifests`** — public-safe property dataset provenance for the PostGIS projection
- **`fairvalue_properties`** — normalized public-safe property rows plus PostGIS geography centroids for future query migration
...
| GET | `/api/neighborhoods/:zipCode/market-drafts` | Playable price-momentum and draft-only neighborhood market scenario contracts |
| GET | `/api/neighborhoods/:zipCode/market-drafts/:draftId/evidence-contract` | Provider-ready neighborhood draft evidence contract |
| POST | `/api/neighborhoods/:zipCode/market-drafts/:draftId/evidence/generate` | Provider-gated neighborhood draft evidence adapter |
...
`GET /api/market-templates` returns the versioned public registry of room market formats. `binary_over_under` uses `lmsr_binary_v1` and is playable in the browser room UI. `range_price_band` uses `lmsr_multi_outcome_v1`: create a room with `market_draft.market_format = "range_price_band"` plus optional `band_low` and `band_high`, place bets on `below_band`, `inside_band`, or `above_band`, and settle with `actual_price`. `rent_yield_over_under` uses `lmsr_binary_v1`: create a room with `market_draft.market_format = "rent_yield_over_under"` plus optional `yield_threshold`, place bets on `over` or `under`, and settle with `actual_price` or `settlement_price` plus `annual_rent`. `time_on_market_over_under` uses `lmsr_binary_v1`: create a room with `market_draft.market_format = "time_on_market_over_under"` plus optional `days_threshold`, place bets on `over` or `under`, and settle with `days_on_market` or `listed_at` plus `contract_at`/`pending_at`. `renovation_budget_over_under` uses `lmsr_binary_v1`: create a room with `market_draft.market_format = "renovation_budget_over_under"` plus optional `budget_threshold`, place bets on `over` or `under`, and settle with `verified_cost`. `neighborhood_price_momentum_over_under` uses `lmsr_binary_v1`: create a room with `market_draft.market_format = "neighborhood_price_momentum_over_under"` plus optional `baseline_median_price`, `price_momentum_threshold`, `comparison_window`, and `zip`, place bets on `over` or `under`, and settle with `future_median_price` backed by public-safe ZIP aggregate evidence. Neighborhood rent-yield and outperformance formats are still published for developer planning and `/api/neighborhoods/:zipCode/market-drafts`; room creation rejects those draft-only formats with a "not playable yet" error. Range, rent-yield, time-on-market, renovation-budget, and neighborhood price-momentum room events include `market_format` and `market_config`, replay/live verification hashes them, public verification exports the settled outcome, and the host/player UI renders format-specific probabilities, positions, and settlement hints. The AI bot remains binary-first for standard asking-price over/under rooms.
...
- **Property PostGIS projection** — optional `fairvalue.propertyPostgresProjection.v1` tables generated from the static snapshot and used as the explicit property read adapter when `FAIRVALUE_PROPERTY_SNAPSHOT_STORE=postgres`
...
- `FAIRVALUE_PROPERTY_SNAPSHOT_PATH` overrides the static property snapshot used by private alert evaluation, default server-side property query surfaces, explicit fallback property reads, and `npm run data:postgres:properties`. If unset, the server reads `public/data/properties.json`.
- `FAIRVALUE_PROPERTY_SNAPSHOT_STORE=postgres` explicitly makes server-side property APIs load the public-safe PostGIS projection from `fairvalue_properties`/`fairvalue_property_manifests` at startup. Leave it unset until `npm run data:postgres:properties -- --write` has populated the tables for the configured `DATABASE_URL`. The older `FAIRVALUE_PROPERTY_QUERY_SOURCE=postgres` alias is still accepted.
- `FAIRVALUE_PROPERTY_POSTGRES_FALLBACK=static` lets the explicit Postgres property read adapter fall back to the static JSON snapshot if the projection is disabled, empty, or unavailable. Leave it unset to fail closed during startup.
- `FAIRVALUE_INTELLIGENCE_PROVIDER_URL`, `FAIRVALUE_INTELLIGENCE_PROVIDER_API_KEY`, and `FAIRVALUE_INTELLIGENCE_PROVIDER_NAME` configure optional server-side structured property intelligence generation. The URL must use HTTPS outside localhost; the API key is never returned to browsers.
- `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_URL`, `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_API_KEY`, and `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_NAME` configure optional server-side neighborhood evidence generation for ZIP scenario markets. Accepted provider output can support playable price-momentum settlement review, but the remaining draft-only neighborhood formats still require pricing, replay, room creation, and settlement workflows before live betting.
...
- `GET /readyz` reports whether the process is ready for its configured dependencies. Local degraded mode is ready without `DATABASE_URL`; `FAIRVALUE_REQUIRE_DATABASE_URL=1` or `FAIRVALUE_ROOM_STORE=postgres` makes the database requirement explicit. It also reports the loaded property snapshot adapter, row count, dataset ID, source hash, and whether the configured source is static JSON or explicit PostGIS projection.
- `GET /api/ops/metrics` returns an in-memory JSON snapshot for local triage: request counts/latency, room lifecycle counters, active room/player/connection counts, WebSocket counters, rate-limit rejections, database errors, persistence failures, property snapshot adapter/count/provenance, and AI degraded/error counts. It does not include room host tokens or player payloads. Set `FAIRVALUE_OPS_TOKEN` before exposing it outside local development.
...
- `GET /api/ops/incidents/:incidentId/replay` returns `fairvalue.operatorIncidentReplayReview.v1`, an ops-token-only redacted replay/live projection check for the incident room. It reports event counts, mismatch counts, projection hashes, settlement/evidence status, summary counts, and bounded redacted canonical event rows without requiring host tokens or returning private room authority, player session IDs, user IDs, or raw evidence documents.
...
- `/ops/incidents` is the frontend operator console over those endpoints. It stores an optional ops token in session storage, renders redacted canonical event-row drilldowns after replay checks, clears raw notes after PATCH, and relies on the server-side workflow redactor for persisted timeline notes.
...
- `GET /metrics` exposes the same aggregate counters in Prometheus text format for an external scraper, including property snapshot loaded/count gauges labeled by adapter. It uses the same `FAIRVALUE_OPS_TOKEN` guard as `/api/ops/metrics`.
...
- `GET /api/properties` returns `fairvalue.propertyQuery.v1` with redacted property rows from the current static snapshot or explicit PostGIS projection read source, filter echoes, match counts, limit, manifest/projection provenance, `source_adapter`, and usage limitations.
...
- `GET /api/geospatial/properties` returns `fairvalue.geospatialIndex.v1` over static provider centroids with approximate distance, 0.01-degree grid cells, coordinate coverage, provenance, and limitations. Supported params: `lat`, `lng`/`lon`, `radius_km`, `bbox` as `west,south,east,north`, explicit `west`, `south`, `east`, `north`, `city`, `state`, `zip` or `zip_code`, and `limit`.
- `GET /api/properties/:propertyId/nearby` returns the same geospatial envelope centered on a snapshot property and excludes the origin property from nearby candidates.
...
- `GET /api/neighborhoods/:zipCode/market-drafts` returns `fairvalue.neighborhoodMarketDrafts.v1` with a playable price-momentum scenario contract plus draft-only rent-yield and benchmark-outperformance contracts. These drafts include static ZIP baselines, default settlement configuration, evidence requirements, trust notices, and explicit limitations.
- `GET /api/neighborhoods/:zipCode/market-drafts/:draftId/evidence-contract` returns `fairvalue.neighborhoodEvidenceProviderContract.v1` for a specific draft ID or market format, including redacted ZIP aggregate context, the draft contract, required `fairvalue.neighborhoodEvidence.v1` output fields, citation requirements, prohibited claims, and a deterministic request hash.
- `POST /api/neighborhoods/:zipCode/market-drafts/:draftId/evidence/generate` returns `fairvalue.neighborhoodEvidenceAdapter.v1`. With `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_URL` and `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_API_KEY`, it posts the contract server-to-server and validates cited aggregate evidence; otherwise it returns local fallback evidence gaps. Provider-backed success can support price-momentum settlement review, but it does not promote the remaining draft-only formats by itself.
- `GET /api/ai/intelligence/properties/:propertyId/contract` returns a redacted `fairvalue.propertyIntelligenceProviderContract.v1` request contract for server-side provider generation.
- `POST /api/ai/intelligence/properties/:propertyId/generate` returns `fairvalue.structuredIntelligenceAdapter.v1` with provider-backed intelligence when configured provider output passes validation, otherwise deterministic local `fairvalue.marketIntelligence.v2` fallback and a provider-attempt reason.
- The API intentionally omits raw provider payloads, photo arrays, street-view URLs, embedded provider keys, host tokens, and user profile state. Geospatial responses are property centroids only; they are not parcel boundaries, assessor/title records, zoning maps, school boundaries, climate/flood layers, or live GIS feeds.
...
server/
  ...
  neighborhoodEvidenceAdapter.js # Provider contract and validation for draft neighborhood evidence
  structuredIntelligenceAdapter.js # Provider contract, execution, validation, and fallback for structured intelligence
