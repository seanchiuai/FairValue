...
- **Private profile and watchlist** — `/me` shows the signed-in player's private prediction history, calibration, simulation-credit portfolio totals, and property watchlist. Signed users sync watchlist items, private notes, and saved price thresholds through `/api/me/watchlist`; the browser still keeps `fv_property_watchlist_v1` as a local fallback.
- **Property data provenance** — `public/data/property-data-manifest.json` is generated from the static provider snapshot with source hashes, field coverage, freshness windows, provider counts, and legal limitations. Property pages surface that data-quality contract beside listing provenance, and `npm run verify` fails if the manifest drifts from `properties.json`.
- **Market template registry** — `/api/market-templates` publishes `market-template-registry/v1`, including playable binary LMSR rooms and rendered range price-band rooms. Rent-yield, time-on-market, and renovation-budget markets remain draft-only and blocked from room creation until their pricing and settlement workflows exist.
...
- **User reputation and profile** aggregate signed-in player calibration plus private watchlist state. Reputation uses `FAIRVALUE_USER_REPUTATION_PATH` or `.fairvalue/user-reputation.json`; watchlists, notes, and saved price thresholds use `FAIRVALUE_USER_PROFILE_PATH` or `.fairvalue/user-profile.json`. `/api/me/reputation` and `/api/me/watchlist` are user-token-protected and omit player session IDs, host tokens, user tokens, and raw evidence. The browser keeps `fv_property_watchlist_v1` as a local fallback and merges local-only items into signed sync when possible.
- **Property data manifest** is generated by `scripts/property-data-manifest.js` into `public/data/property-data-manifest.json`. It treats the current Zillow/MLS export as a static provider snapshot, stores deterministic SHA-256 provenance, and gives frontend surfaces a typed freshness/coverage contract without claiming a live feed.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. `binary_over_under` and `range_price_band` are playable in the rendered room UI, canonical event replay, settlement, and public verification. Remaining future formats are explicit `draft_only` contracts so clients can plan without pretending unsupported workflows exist.
...
- `src/lib/multiOutcomeMarketEngine.js` provides the generic `lmsr_multi_outcome_v1` core for range/ranked templates: n-outcome log-sum-exp cost, softmax probabilities, budget-to-shares execution, public state projection, and winning-outcome settlement math. `range_price_band` rooms use it in the server API path and rendered host/player room UI today; the AI bot remains binary-first.
...
`GET /api/market-templates` returns the versioned public registry of room market formats. `binary_over_under` uses `lmsr_binary_v1` and is playable in the browser room UI. `range_price_band` uses `lmsr_multi_outcome_v1`: create a room with `market_draft.market_format = "range_price_band"` plus optional `band_low` and `band_high`, place bets on `below_band`, `inside_band`, or `above_band`, and settle with `actual_price`. Range room events include `market_format` and `market_config`, replay/live verification hashes them, public verification exports the settled outcome, and the host/player UI renders range probabilities, positions, and settlement hints. The AI bot remains binary-first for now. `rent_yield_over_under`, `time_on_market_over_under`, and `renovation_budget_over_under` remain draft-only contracts.
...
- All `/api/me/*` routes require `X-FairValue-User-Token`. They are private profile state, not public market evidence or notification-delivery guarantees.
...
`npm run check:bundle` defaults to 240 kB for any JS chunk, 25 kB for any CSS chunk, and 790 kB total JS after `npm run build`. Override with `FAIRVALUE_MAX_JS_CHUNK_KB`, `FAIRVALUE_MAX_CSS_CHUNK_KB`, or `FAIRVALUE_MAX_TOTAL_JS_KB` when intentionally raising a budget.
