...
- **Room and user reputation** — Settled rooms produce a `room-reputation/v1` summary from public bet history, public reason counts, final outcome accuracy, and Brier-style calibration. Signed-in players also accumulate a private `fairvalue.userReputation.v1` cross-room projection behind their user token. Public artifacts deliberately exclude session IDs, host/user tokens, and private evidence.
