...
- **Market template registry** — `/api/market-templates` publishes `market-template-registry/v1`, including playable binary LMSR, rendered range price-band, rendered rent-yield, rendered time-on-market, and rendered renovation-budget over/under rooms, plus draft-only neighborhood scenario formats that are registered but blocked from live room creation.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. `binary_over_under`, `range_price_band`, `rent_yield_over_under`, `time_on_market_over_under`, and `renovation_budget_over_under` are playable in the rendered room UI, canonical event replay, settlement, and public verification. `neighborhood_price_momentum_over_under`, `neighborhood_rent_yield_over_under`, and `neighborhood_outperformance_over_under` are explicit `draft_only` contracts: they can be generated as scenario drafts from ZIP baselines, but room creation rejects them until neighborhood pricing, evidence ingestion, replay, and settlement workflows exist.
...
- **Neighborhood Evidence Provider Contract** is exposed per draft at `GET /api/neighborhoods/:zipCode/market-drafts/:draftId/evidence-contract`. `POST /api/neighborhoods/:zipCode/market-drafts/:draftId/evidence/generate` can send a redacted ZIP aggregate contract to `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_URL` with `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_API_KEY`, validate cited `fairvalue.neighborhoodEvidence.v1` aggregate evidence, and fall back to local gap assessment when credentials are absent or output fails. Property detail scenario cards render the adapter readiness. Accepted evidence marks the draft as evidence-adapter-ready only; pricing, replay, room creation, settlement UI, and disputes remain blocked.
...
| GET | `/api/neighborhoods/:zipCode/market-drafts` | Draft-only neighborhood market scenario contracts |
...
- `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_URL`, `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_API_KEY`, and `FAIRVALUE_NEIGHBORHOOD_EVIDENCE_PROVIDER_NAME` configure optional server-side neighborhood evidence generation for draft-only ZIP scenario markets. Accepted provider output can mark a draft `evidence_adapter_ready`, but pricing, replay, room creation, and settlement workflows are still required before live betting.
...
- `GET /api/neighborhoods/:zipCode/market-drafts` returns `fairvalue.neighborhoodMarketDrafts.v1` with draft-only price-momentum, rent-yield, and benchmark-outperformance scenario contracts. These drafts include static ZIP baselines, default settlement configuration, evidence requirements, trust notices, and explicit limitations, but they are not playable room formats.
