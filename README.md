...
- **Market template registry** — `/api/market-templates` publishes `market-template-registry/v1`, including playable binary LMSR, rendered range price-band, and rendered rent-yield over/under rooms. Time-on-market and renovation-budget markets remain draft-only and blocked from room creation until their pricing and settlement workflows exist.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. `binary_over_under`, `range_price_band`, and `rent_yield_over_under` are playable in the rendered room UI, canonical event replay, settlement, and public verification. Remaining future formats are explicit `draft_only` contracts so clients can plan without pretending unsupported workflows exist.
...
Signed example artifacts live at `docs/fixtures/public-room-verification-v1.json`, `docs/fixtures/public-room-verification-range-price-band-v1.json`, and `docs/fixtures/public-room-verification-rent-yield-v1.json` so future SDK, webhook, and embed consumers can lock against binary, range, and rent-yield export shapes without needing live room credentials.
