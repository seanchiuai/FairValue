...
- **Private profile and watchlist** — `/me` shows the signed-in player's private prediction history, calibration, simulation-credit portfolio totals, and browser-local property watchlist. Property pages include a watch control backed by `fv_property_watchlist_v1` in local storage.
...
- **User reputation** aggregates signed-in player calibration across settled rooms in a private store (`FAIRVALUE_USER_REPUTATION_PATH` or `.fairvalue/user-reputation.json` in the real server process). The `/api/me/reputation` response is user-token-protected, powers the `/me` private profile/history route, and omits player session IDs, host tokens, user tokens, and raw evidence. The property watchlist is browser-local state under `fv_property_watchlist_v1`, so it is inspectable in `/me` without claiming server-backed sync.
...
`npm run check:bundle` defaults to 240 kB for any JS chunk, 25 kB for any CSS chunk, and 785 kB total JS after `npm run build`. Override with `FAIRVALUE_MAX_JS_CHUNK_KB`, `FAIRVALUE_MAX_CSS_CHUNK_KB`, or `FAIRVALUE_MAX_TOTAL_JS_KB` when intentionally raising a budget.
