...
- **Multiplayer** — Host creates a room at `/join`, gets a 4-character code using letters and numbers. Players scan QR or go to `/play/:roomCode` to bet from their phones with a local LMSR pre-bet read, reason to believe, reason to doubt, and wager-impact preview. Host views live dashboard at `/host/:roomCode` with chart, leaderboard, activity feed, phase/timer/lock controls, and Live Room Intelligence generated from LMSR flow, players, recent bets, and optional Market Studio draft audit metadata.
