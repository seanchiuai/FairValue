...
- **Market template registry** — `/api/market-templates` publishes `market-template-registry/v1`, including the playable binary LMSR format and draft-only contracts for range, rent-yield, time-on-market, and renovation-budget markets. Draft-only formats are registered but blocked from room creation until pricing and settlement engines exist.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. Only `binary_over_under` is marked `playable`; future formats are explicit `draft_only` contracts so clients can plan without pretending unsupported engines exist.
...
`GET /api/market-templates` returns the versioned public registry of room market formats. `binary_over_under` is the only playable format today and uses `lmsr_binary_v1`; `range_price_band`, `rent_yield_over_under`, `time_on_market_over_under`, and `renovation_budget_over_under` are registered as draft-only contracts until their pricing engines, settlement inputs, replay rules, and UI workflows are implemented.
