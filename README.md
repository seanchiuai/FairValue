...
- **Room and user reputation** — Settled rooms produce a `room-reputation/v1` summary from public bet history, public reason counts, final outcome accuracy, and Brier-style calibration. Signed-in players also accumulate a private `fairvalue.userReputation.v1` cross-room projection behind their user token, surfaced on the player room view after settlement. Public artifacts deliberately exclude session IDs, host/user tokens, and private evidence.
- **Market template registry** — `/api/market-templates` publishes `market-template-registry/v1`, including playable binary LMSR rooms and API-playable range price-band rooms. Rent-yield, time-on-market, and renovation-budget markets remain draft-only and blocked from room creation until their pricing and settlement workflows exist.
...
- **Market Template Registry** is a versioned contract in `src/data/marketTemplates.json`, exposed publicly at `/api/market-templates`. `binary_over_under` is fully playable in the rendered room UI; `range_price_band` is playable through the room API, canonical event replay, settlement, and public verification. Remaining future formats are explicit `draft_only` contracts so clients can plan without pretending unsupported workflows exist.
...
- `src/lib/multiOutcomeMarketEngine.js` provides the generic `lmsr_multi_outcome_v1` core for range/ranked templates: n-outcome log-sum-exp cost, softmax probabilities, budget-to-shares execution, public state projection, and winning-outcome settlement math. `range_price_band` rooms use it on the server API path today; the rendered host/player room UI and AI bot remain binary-first.
...
`GET /api/market-templates` returns the versioned public registry of room market formats. `binary_over_under` uses `lmsr_binary_v1` and is playable in the browser room UI. `range_price_band` uses `lmsr_multi_outcome_v1` and is playable through the room API: create a room with `market_draft.market_format = "range_price_band"` plus optional `band_low` and `band_high`, place bets on `below_band`, `inside_band`, or `above_band`, and settle with `actual_price`. Range room events include `market_format` and `market_config`, replay/live verification hashes them, and public verification exports the settled outcome. The current rendered host/player UI and AI bot remain binary-first, so API clients should treat range rooms as server-supported but not yet first-class in the visual room workflow. `rent_yield_over_under`, `time_on_market_over_under`, and `renovation_budget_over_under` remain draft-only contracts.
