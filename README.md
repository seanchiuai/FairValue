...
- `GET /api/ops/incidents` returns `fairvalue.operatorIncidentQueue.v1`, a redacted deterministic triage queue over current room/review state. Supported params: `room_code`, `severity`, and `limit`. It uses the same `FAIRVALUE_OPS_TOKEN` guard as metrics and is not moderation enforcement, arbitration, legal advice, appraisal authority, fraud finding, or compliance review.
