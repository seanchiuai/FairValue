...
- **Property PostGIS projection** — optional `fairvalue.propertyPostgresProjection.v1` tables generated from the static snapshot for future database-backed property APIs
...
- `FAIRVALUE_PROPERTY_SNAPSHOT_PATH` overrides the static property snapshot used by private alert evaluation, server-side property query surfaces, and `npm run data:postgres:properties`. If unset, the server reads `public/data/properties.json`.
