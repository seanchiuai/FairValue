...
- On 2026-08-04 critical room mutations gained a rollback boundary: failed durable writes remove the appended event where the adapter supports removal and restore the room snapshot before returning `503`; focused rollback/event-log coverage passed 22 tests and `npm run test:server` passed 115 tests.
