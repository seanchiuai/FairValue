...
- On 2026-08-04 the focused negative-path browser suite passed 27 Chromium scenarios on fresh backend/frontend ports, including the unauthorised `/review/:roomCode` branch, transient room-state Retry recovery, the player client wager-rate-limit branch, and incomplete Market Studio draft validation: public evidence remained visible, the host-only events endpoint was not requested, the private host token was absent from rendered content, a retry restored the room after a transient 503, the sixth rapid wager was blocked before network mutation, incomplete drafts stayed out of the room-creation network path, and all exercised states passed serious/critical axe checks. The focused `RateLimiter` unit suite passed 2 tests.
