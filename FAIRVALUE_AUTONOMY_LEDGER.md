...
- Host-only settlement and AI toggles are not yet protected by a capability token.
...
1. Protect host-only actions with a host token and tests.
2. Align room-code generation, UI validation, and API tests.
3. Add API/WebSocket tests for create room, join, bet, leaderboard, and settlement.
4. Add idempotency keys, validation, rate limits, and request correlation IDs.
5. Create an append-only room event log with replay tests.
6. Unify backend and frontend LMSR logic behind one shared domain boundary.
7. Add Playwright E2E coverage for host/player room flow.
8. Address npm audit vulnerabilities without breaking CRA compatibility.
...
1. Protect host-only actions with a host capability token for settlement and AI toggles.
2. Align room-code generation, join validation, UI copy, and API tests.
3. Add backend API/WebSocket integration tests for joins, bets, settlement, and reconnect state recovery.
4. Add idempotency keys and payload validation for betting.
5. Start the next loop with `npm run verify`, then attack host authority.
