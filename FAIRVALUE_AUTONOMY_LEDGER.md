...
- On 2026-08-04 the focused negative-path browser suite passed 28 Chromium scenarios on fresh backend/frontend ports, including the unauthorised `/review/:roomCode` branch, transient room-state Retry recovery, incomplete Market Studio draft validation, host phase failure handling, and the player client wager-rate-limit branch: public evidence remained visible, the host-only events endpoint was not requested, the private host token was absent from rendered content, a retry restored the room after a transient 503, incomplete drafts stayed out of the room-creation network path, malformed and failed phase writes left betting open with controls re-enabled, the sixth rapid wager was blocked before network mutation, and all exercised states passed serious/critical axe checks. The focused `RateLimiter` unit suite passed 2 tests.
