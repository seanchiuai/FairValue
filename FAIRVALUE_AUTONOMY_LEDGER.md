...
- Runtime dependencies are now separated from frontend test/type tooling after migrating off CRA/react-scripts; current `npm audit --omit=dev --json` still reports two high entries for React Router's RSC-mode advisory. FairValue uses BrowserRouter only, and `SECURITY.md` records the bounded exposure and release gate.
...
- On 2026-08-04 the isolated browser suite passed all 48 Chromium scenarios on fresh backend/frontend ports, including host/player recovery, room-format flows, profile/recap/export, negative-path notifications, expanded serious/critical axe coverage, and the comparison workflow.
...
- On 2026-08-04 a live macOS VoiceOver-assisted pass ran against the existing frontend `3018` and backend `8018`: browse, sort, property detail, join selection, create-room fields, host controls, and the host AI panel were reached through the rendered Chrome surface; the visible AI conversation showed `Evidence used:` and `Limits:`. VoiceOver was returned to off after capture. The caption-panel/rotor state was inspected, but no human listener or reliable speech-output readback was available, so the manual VoiceOver listening/rotor gate remains open.
- On 2026-08-04 `npm run test:persistence:postgres` passed against disposable Docker `postgres:16-alpine`, including the Postgres room adapter, append-only event adapter, and settled-row retention pruning; `npm run test:persistence:live` returned an explicit skipped result because `DATABASE_URL` is unset.
- On 2026-08-04 `npm run check:production` failed honestly with 7 deployment failures, 3 warnings, and 1 pass because Postgres, identity, ops, public-verification, retention, and optional Cognee environment values are not configured in this workspace; no production readiness claim was made.
- On 2026-08-04 the focused negative-path browser suite passed 29 Chromium scenarios on fresh backend/frontend ports, including direct-player identity outage handling, the unauthorised `/review/:roomCode` branch, transient room-state Retry recovery, incomplete Market Studio draft validation, host phase failure handling, and the player client wager-rate-limit branch: identity failure stayed out of the room-join network path, public evidence remained visible, the host-only events endpoint was not requested, the private host token was absent from rendered content, a retry restored the room after a transient 503, incomplete drafts stayed out of the room-creation network path, malformed and failed phase writes left betting open with controls re-enabled, the sixth rapid wager was blocked before network mutation, and all exercised states passed serious/critical axe checks. The focused `RateLimiter` unit suite passed 2 tests.
- On 2026-08-04 the React Router audit boundary was rechecked against the live npm registry: the safe `7.18.2` line is now the minimum manifest range, while npm's suggested `7.11.0` downgrade was rejected because that older release carries additional high and moderate advisories. The remaining two high audit entries describe RSC request handling, which FairValue does not enable; `SECURITY.md` records the BrowserRouter-only exposure and keeps release-owner acceptance or a router replacement as an explicit production gate.
- On 2026-08-04 critical room mutations and durable failure events gained the same serialized per-room execution boundary: failed durable writes remove the appended event where the adapter supports removal and restore the room snapshot before returning `503`; validation, authorization, and mutation errors now await their audit persistence instead of racing later room writes. Focused rollback/serialization coverage passed 24 tests and `npm run test:server` passed 117 tests.
- On 2026-08-04 the browse surface's light-background accent labels were corrected after axe reported serious contrast failures; the expanded accessibility scenario and the full 48-test suite now pass.
...
1. Continue deeper branch-level coverage beyond the now-passing 48-test Chromium suite, prioritizing validation and notification states added after the current browser matrix.
