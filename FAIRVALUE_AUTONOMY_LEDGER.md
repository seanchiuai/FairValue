...
- Player pre-bet balance-capped warnings now expose a stable rendered test hook and polite live semantics, with browser coverage proving an over-balance selected wager previews against the player's remaining simulation-credit balance.
...
- 2026-05-16 Player Pre-Bet Balance Cap pass: `npm run typecheck` passed; focused `npm test -- playerBetPreview` passed 1 file / 2 tests; focused Playwright passed the new balance-capped preview spec with room `YRY0`; rendered mobile probe on backend `8092` / frontend `3092` created room `ZY1D`, joined `/play/ZY1D`, placed a `$950` OVER wager, verified remaining balance `50`, verified `Preview capped at your current $50 balance.`, no active alerts, no horizontal overflow, and captured the card at `/tmp/fairvalue-prebet-balance-cap-card-mobile.png` with one benign dev WebSocket close warning during initial connection; full `npm run test:e2e:isolated` passed 34 Chromium tests; final `npm run verify` passed client secret scan, typecheck, 44 server tests, 12 Vitest suites / 65 tests, production build, bundle budget with total JS 736.17 kB / 760.00 kB, and `smoke:boot` room `V3X6`.
...
### 2026-05-16 - Player Pre-Bet Balance Cap Coverage
...
- Captured rendered mobile card proof for the capped-warning state after joining and betting through the real `/play/:roomCode` UI.
