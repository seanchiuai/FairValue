...
- On 2026-08-04 critical room mutations gained serialized per-room execution plus a rollback boundary: failed durable writes remove the appended event where the adapter supports removal and restore the room snapshot before returning `503`; focused rollback/serialization coverage passed 23 tests and `npm run test:server` passed 116 tests.
